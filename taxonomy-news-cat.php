<?php get_header(); ?> <!-- header.phpを読み込むテンプレートタグ -->
        <article>
          <div class="p-mainvisual p-mainvisual--img-archive"> <!-- flex column center center タテヨコ真ん中 -->
            <h1 class="p-mainvisual__title">News:</h1>
            <p class="p-mainvisual__subtitle"><?php echo single_cat_title('', false), "（", $wp_query->found_posts, "件）"; ?></p> <!-- カテゴリー名を表示 -->
          </div>
            ...
            <ul>
              <?php if(have_posts()) : // 投稿データがあるか確認
                while(have_posts()) :
                  the_post(); // 投稿データの取得 ?>
                  <li class="p-card">
                    <div class="p-card__img p-card__img--news"><img src="<?php bloginfo('template_directory'); ?>/img/news.jpg" alt="news icon"></div> <!-- アイキャッチ画像を出力 -->
                    <div class="p-card__content p-card__content--news">
                      <?php
                        $days = 14;
