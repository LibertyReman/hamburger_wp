            ...
            while(have_posts()) :
              ...
              <div id="post-<?php the_ID(); ?>" <?php post_class(); ?>> <!-- 投稿情報classを追加 -->
                ...
                <div class="l-main__wrapper">
                  ...
                  <div class="p-suggest">
                    <h2 class="p-suggest__title"><i class="fas fa-star"></i> おすすめ情報</h2>
                    <?php
                      ...
                      if($suggest_title and $suggest_link) : // おすすめタイトルとリンクがある場合
                        echo '<a class="p-suggest__text-link" href="', esc_url($suggest_link), '">', esc_html($suggest_title), '</a>';
                      elseif($suggest_title and $suggest_link = " ") : // おすすめタイトルのみある場合
                        echo '<p class="p-suggest__text">', esc_html($suggest_title), '</p>';
                      else :
                        $category = get_the_category();
                        $link = $category[2]->term_id ? $category[2]->term_id :  $category[0]->term_id;
                        echo '<a class="p-suggest__text-link" href="', get_category_link($link), '">ブログのトップページへ</a>';
                      endif;
                    ?>

                    <?php
                      if (function_exists('yarpp_related')) {
                        related_posts(); // 関連記事の表示
                      }
                    ?>
                  </div> <!-- .p-suggest -->
                </div> <!-- .l-main__wrapper -->
